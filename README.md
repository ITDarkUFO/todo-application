...
## Начальная настройка 
Запуск можно проводить как через Visual Studio, так и через Docker (имеется ввиду запуск в Debug версии через Visual Studio), но перед первым запуском необходимо выполнить несколько действий по настройке:
